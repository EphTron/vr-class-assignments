      ...
      if _name.count("monkey") > 0: # a monkey node
        ...
        if self.is_highlight_material(_node.CurrentColor.value): # monkey node in close proximity
          ...
          if self.dragging_technique == 1: # change of node order in scenegraph
            ...
            self.offset = avango.gua.make_inverse_mat(self.hand_transform.WorldTransform.value) * _node.Transform.value
  ...
  def object_dragging(self):
    ...
    for _node in self.dragged_objects:
      ...
      if self.dragging_technique == 1: # change of node order in scenegraph
        ...
        self.diff_mat = avango.gua.make_inverse_mat(self.last_frame) * self.hand_transform.WorldTransform.value
        self.last_frame = self.hand_transform.WorldTransform.value
